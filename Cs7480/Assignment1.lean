...
Problem 1: $\mathsf{Formula}$: the category of syntactic Boolean formulae and entailment]
  ...
  if $\mathsf{Mods}(\varphi) \subseteq \mathsf{Mods}(\psi)$.
    ...
    Consider the following components of a category whose objects are Boolean formula and whose morphisms denote formula entailment:
    \begin{itemize}
    ...
    \end{itemize}
    Verify that the above satisfy the definition of a category. This will involve showing that some entailments are
    valid.
-/
